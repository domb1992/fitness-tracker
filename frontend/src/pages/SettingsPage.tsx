...
function passwordStrength(pw: string): { level: 0 | 1 | 2 | 3; label: string; color: string } {
  ...
  if (pw.length < 8)   return { level: 1, label: 'Weak',   color: 'oklch(0.58 0.20 25)'  };
  if (pw.length < 12)  return { level: 2, label: 'Medium', color: 'oklch(0.70 0.15 55)'  };
  return                      { level: 3, label: 'Strong',  color: 'oklch(0.55 0.18 145)' };
}
...
function InfoBanner({ children }: { children: ReactNode }) {
  return (
    <div style={{
      ...
      fontSize: 13, color: 'oklch(0.38 0.15 145)', lineHeight: 1.45,
    }}>
      {children}
    </div>
  );
...
function ErrorBanner({ children }: { children: ReactNode }) {
  return (
    <div style={{
      ...
      borderRadius: 'var(--r-1)', padding: '10px 12px',
      fontSize: 13, color: 'oklch(0.50 0.22 25)',
    }}>
      {children}
    </div>
  );
    ...
    <div className="ft-screen" style={{ paddingBottom: 'var(--nav-safe)' }}>
      ...
      <div style={{ padding: '0 20px', display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        <div className="surface" style={{ padding: '16px 16px' }}>
          <div className="mono-tag" style={{ marginBottom: 14 }}>Profile</div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            ...
            {emailConfirmSent && (
              <InfoBanner>
                A confirmation link has been sent to <strong>{email.trim().toLowerCase()}</strong>.
            ...
            <button
              ...
              className="block-btn"
              style={{ height: 48, fontSize: 14, opacity: profileSaving || !profileChanged ? 0.4 : 1 }}
            >
              <span>
        ...
        {/* ── Change Password ──────────────────────────────────────────────── */}
        <div className="surface" style={{ padding: '16px 16px' }}>
          <div className="mono-tag" style={{ marginBottom: 14 }}>Change Password</div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            ...
            <div>
              ...
              {/* Strength bar — only show once the user starts typing */}
              {newPw.length > 0 && (
                <div style={{ marginTop: 6 }}>
                  <div style={{
                    height: 3, borderRadius: 2, background: 'var(--paper-3)', overflow: 'hidden',
                  }}>
                    <div style={{
            ...
            <div>
              ...
              {confirmPw.length > 0 && newPw.length > 0 && (
                <div style={{
                  ...
                  letterSpacing: '0.06em', textTransform: 'uppercase',
                  color: newPw === confirmPw ? 'oklch(0.55 0.18 145)' : 'oklch(0.58 0.20 25)',
                }}>
            ...
            <button
              ...
              className="block-btn"
              style={{ height: 48, fontSize: 14, opacity: pwSaving || !newPw || !confirmPw ? 0.4 : 1 }}
            >
            ...
            {/* Reset via email (kept as fallback) */}
            {resetSent ? (
              <InfoBanner>
                Check your email — a reset link has been sent to <strong>{user?.email}</strong>.
              </InfoBanner>
            ) : (
              <>
        ...
        <div className="surface" style={{ padding: '16px 16px' }}>
          ...
          <button
            ...
            style={{
              ...
              border: '1px solid oklch(0.55 0.22 25 / 0.20)', borderRadius: 'var(--r-2)',
        ...
        {/* ── Version ──────────────────────────────────────────────────────── */}
        <div style={{ textAlign: 'center', paddingTop: 2, paddingBottom: 8 }}>
          <span style={{
